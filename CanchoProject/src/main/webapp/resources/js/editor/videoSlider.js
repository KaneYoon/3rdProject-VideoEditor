...
function getFileType(path){
	...
	var vExtarr = new Array('mp4', 'wemb', 'ogg');
	var aExtarr = new Array('mp3');
	var rtn = 'file';
	...
	for (var i in vExtarr) {
		if(vExtarr[i] == ext.toLowerCase())
			return 'video';
	}
	for (var i in aExtarr) {
		if(aExtarr[i] == ext.toLowerCase())
			return 'audio';
	}
	return rtn;
}
 ...
 * 비디오트랙 클릭 이벤트 등록
 */
function MainTrackEventRegister(){
 ...
 * 비디오트랙 클릭 이벤트 해제
 */
function MainTrackEventRemover(){
	$('.video-obj').off('click');	
}

function UnSeletedObj(){
	var IdsOftrackObjs = $.makeArray($(".track-obj").map(function(){
	    return $(this).attr("id");
	}));
	for(var i = 0; i < IdsOftrackObjs.length; i++){
...
function addMainTrackVideoObj(id){
	...
	var fname = getFileName(path);	
		...
		success : function(videoInfo) {
			...
			var newid = 'video'+'Obj' + mainTrackobjNum;
			mainTrackobjNum++;
...
function resizeMainTrackObj(maxValue){
	...
	for(var i = 0; i < totalNumberOfObj.length; i++){
		...
		var newWidth = ( width * thisCnt ) / maxValue;
		$('#'+thisId).css({
...
function resizeSubTrackObj(id, trackId){	
	...
	var newWidth = ( width * thisCnt ) / totalVideoTrack;
...
function updateLocationObj(id, trackId){
	...
	var unitDuration = maxDuration / width;
